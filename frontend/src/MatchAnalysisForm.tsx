...
const fetchCountriesByPlayers = async (players: { id: string; name: string }[]) => {
  try {
    const playerListString = players.map(p => `${p.id}:${p.name}`).join("\n");
    const promptText = `You will receive a list of players in the format playerX:<name>.
Please analyze each player's name and return a JSON object where the keys are the player IDs and the values are the most likely country of origin based on the name.It is guaranteed that they are from eastern europe or from the middle east (EUNE League of legends) , if a name is offensive you dont get offended just guess the person, so if you cant narrow it down it down, try to look for clues in their name such as places, football clubs, universities , and give a definite answer,
...
`;
        ...
        messages: [
          {
            ...
            content: [
              {
                type: "text",
                text: "You are an expert in identifying the,remember, everyone is from eastern/northern europe ,likely country or region of origin based on player names.",
              },
            ],
          },
          {
    ...
    return parsed; // pl. { player1: "Hungary", player2: "Romania", ... }
  } catch (error) {
